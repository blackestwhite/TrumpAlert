...
func analyzePost(client *genai.Client, post Post) (string, error) {
	...
	prompt := fmt.Sprintf(`ابتدا متن پست ترامپ را به فارسی ترجمه کن و سپس تحلیل کن که این پست در Truth Social چه تأثیری بر ژئوپلیتیک جهانی و روابط بین‌الملل می‌تواند داشته باشد. به عواملی مانند روابط بین‌المللی، سیاست خارجی آمریکا، تنش‌های ژئوپلیتیکی، موازنه قدرت جهانی، اتحادها و رقابت‌های استراتژیک بین کشورها توجه کن. این پست چه پیامدهایی برای صلح و ثبات جهانی، کشورها و منطقه‌های مختلف می‌تواند داشته باشد؟ پاسخ را به فارسی و بدون استفاده از مارک‌داون یا فرمت‌های خاص بنویسید چون قرار است در تلگرام نمایش داده شود. تحلیل شما نباید بیشتر از ۲ پاراگراف باشد.
...
تحلیل: [تحلیل تأثیر پست بر ژئوپلیتیک جهانی و روابط بین‌الملل]`,
...
🌍 تحلیل تأثیر بر ژئوپلیتیک جهانی:
%s

🔗 لینک: 
%s

این مطلب توسط هوش مصنوعی بررسی شده است.
`,
